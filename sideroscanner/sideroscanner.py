...
def parse_args():
    parser = ArgumentParser(add_help=False,
                                     ...
                                     usage="scripts.py -i query.fasta",
                                     description='''
...
def main():
    ...
    if not path.isfile(lib):
        exit(f'[!] {lib} is not a valid file \n'
              f'You can download the current IROMP HMM library from:'
              f'https://github.com/tomdstanton/sideroscanner/blob/master/databases/HMMs/iromps.hmm')
    else:
    ...
    for seq in parse_args().i:
        ...
        if parse_args().e is not None:
            ...
            export_proteins(iromps, parse_args().e, hits, seq)
